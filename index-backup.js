...
(async () => {
  ...
  // Extract repository links only for commit events
  const repoLinks = await page.evaluate(() => {
    const anchors = Array.from(document.querySelectorAll('a[data-hovercard-type="repository"]'));
    const filtered = anchors.filter(anchor => {
      const li = anchor.closest('li');
      return li && li.querySelector('a[href*="/commits?"]') !== null;
    });
    return [...new Set(filtered.map(anchor => anchor.href))];
  });
